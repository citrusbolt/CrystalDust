...
static void PrintNumPlayersInLink(u16 windowId, u32 numPlayers)
{
    ...
    xPos = GetStringCenterAlignXOffset(2, gStringVar4, 88);
    AddTextPrinterParameterized(windowId, 2, gStringVar4, xPos, 1, 0xFF, NULL);
...
static void Task_LinkupExchangeDataWithLeader(u8 taskId)
{
    ...
    else
    {
        ...
        TrainerCard_GenerateCardToSendInLink(card);
...
static void Task_StartWiredCableClubBattle(u8 taskId)
{
    ...
    case 5:
        if (gLinkPlayers[0].trainerId & 1)
            PlayMapChosenOrBattleBGM(MUS_VS_JOHTO_LEADER);
        else
            PlayMapChosenOrBattleBGM(MUS_VS_JOHTO_TRAINER);
    ...
    case 3:
        if (GetBlockReceivedStatus() == GetLinkPlayerCountAsBitFlags())
        {
            for (i = 0; i < GetLinkPlayerCount(); i++)
            {
                ...
                NormalizeLinkPlayer(&gLinkPlayers[i]);
